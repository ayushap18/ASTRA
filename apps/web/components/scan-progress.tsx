...
const STAGES = [
  { key: "repository_ingestion", label: "Ingest" },
  { key: "ecosystem_detection", label: "Manifest" },
  { key: "dependency_resolution", label: "Resolve" },
  { key: "metadata_enrichment", label: "Registry" },
  { key: "vulnerability_matching", label: "OSV" },
  { key: "reachability_analysis", label: "Reach" },
  { key: "risk_aggregation", label: "Risk" },
...
export function ScanProgress({ scanId, initial }: { scanId: string; initial: ScanEvent[] }) {
  ...
  return (
    <div className="process" aria-live="polite">
      <div className="process-rail">
        <span className="process-fill" style={{ width: `${width}%` }} />
      </div>
      <ol className="process-steps">
        {STAGES.map((stage, step) => (
          <li key={stage.key} className={failed ? "" : step <= index ? "done" : ""}>
            {stage.label}
          </li>
        ))}
      </ol>
      <p className="caption">
        {latest.stage} · {latest.progress}% · {latest.message}
      </p>
    </div>
  );
}
